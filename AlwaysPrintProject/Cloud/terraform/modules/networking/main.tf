...
# Subnets publicas (EC2 + ALB si se necesita)
...
# Subnets de base de datos (RDS - aisladas, sin internet)
...
resource "aws_security_group" "ec2" {
  ...
  tags = { Name = "${local.prefix}-sg-ec2" }
}

# RDS: solo desde el EC2
resource "aws_security_group" "rds" {
  name        = "${local.prefix}-sg-rds"
  description = "RDS PostgreSQL - traffic from EC2 only"
  vpc_id      = aws_vpc.main.id
