...
async def get_workstation_connectivity(
    ...
    # Verificar tenant isolation: workstation debe pertenecer a la organización del usuario
    ...
    if current_user.organization_id:
        ...
        # Admin sin organization_id puede ver todas las workstations
        workstation = db.query(Workstation).filter(
            Workstation.id == workstation_id
        ).first()
    ...
    records = connectivity_service.get_connectivity_history(
        ...
        organization_id=str(workstation.organization_id) if workstation.organization_id else str(current_user.organization_id or ""),
        check_id=check_id,
        from_dt=from_dt,
        to_dt=to_dt,
        limit=limit
