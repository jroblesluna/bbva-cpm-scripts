...
Migración consolidada que incluye el esquema completo de la base de datos.
Fusiona las migraciones 001-007 originales en una sola migración inicial:
...
- devices (impresoras registradas)
- global_configs, vlan_configs, workstation_configs (configuración jerárquica)
- messages, message_deliveries (mensajería y entregas)
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # Columna de migración 003: auto_reregister_enabled
        sa.Column('auto_reregister_enabled', sa.Boolean(), nullable=False, server_default='false'),
        # Columna de migración 005: forced_contingency a nivel organización
    ...
    op.create_table(
        ...
        # Columna de migración 005: forced_contingency a nivel VLAN
        sa.Column('forced_contingency', sa.Boolean(), nullable=False, server_default='false'),
        sa.Column('created_at', sa.DateTime(), nullable=False, server_default=sa.text('CURRENT_TIMESTAMP')),
        sa.Column('updated_at', sa.DateTime(), nullable=False, server_default=sa.text('CURRENT_TIMESTAMP')),
        sa.ForeignKeyConstraint(['organization_id'], ['organizations.id'], ondelete='CASCADE'),
    ...
    op.create_index('ix_devices_vlan_id', 'devices', ['vlan_id'])
    ...
    op.create_table(
        ...
        # Columnas de migración 002: cidr y tray_version
        sa.Column('cidr', sa.String(length=45), nullable=True),
        sa.Column('tray_version', sa.String(length=50), nullable=True),
        ...
        # Columnas de migración 006: contingency_ip y contingency_started_at
        sa.Column('contingency_ip', sa.String(length=45), nullable=True),
        ...
        sa.ForeignKeyConstraint(['organization_id'], ['organizations.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['vlan_id'], ['vlans.id'], ondelete='SET NULL'),
        sa.ForeignKeyConstraint(['default_printer_id'], ['devices.id'], ondelete='SET NULL'),
        sa.PrimaryKeyConstraint('id'),
    ...
    # === TABLA: messages (con columnas de migración 007: target_type y delivery_mode) ===
    op.create_table(
        'messages',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('organization_id', postgresql.UUID(as_uuid=True), nullable=False),
        ...
        sa.Column('delivery_mode', sa.Enum('all', 'only_connected', name='deliverymode'), nullable=False, server_default='all'),
    ...
    # === TABLA: message_deliveries (migración 007) ===
    op.create_table(
        'message_deliveries',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('message_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('workstation_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('status', sa.Enum('pending', 'sent', 'skipped', name='deliverystatus'), nullable=False, server_default='pending'),
        sa.Column('delivered_at', sa.DateTime(), nullable=True),
...
def downgrade() -> None:
    """Eliminar todas las tablas, funciones y tipos del sistema en orden inverso."""

    connection = op.get_bind()

    # Eliminar funciones PostgreSQL primero
    if connection.dialect.name == 'postgresql':
