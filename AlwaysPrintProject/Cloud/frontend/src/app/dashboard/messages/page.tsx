...
export default function MessagesPage() {
  ...
  const loadMessages = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/v1/messages/?${params.toString()}`, { headers: getAuthHeaders() })
  ...
  const loadStats = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/v1/messages/stats', { headers: getAuthHeaders() })
      if (!response.ok) throw new Error('Error')
      const data = await response.json()
      setStats(data)
    } catch (error) {
...
function SendMessageModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  useEffect(() => {
    const loadWS = async () => {
      try {
        const r = await fetch('http://localhost:8000/api/v1/workstations/', { headers: getAuthHeaders() })
        if (r.ok) { const d = await r.json(); setWorkstations(d.items || []) }
      } catch (e) { console.error(e) }
    }
    const loadVLANs = async () => {
      try {
        const r = await fetch('http://localhost:8000/api/v1/vlans/', { headers: getAuthHeaders() })
