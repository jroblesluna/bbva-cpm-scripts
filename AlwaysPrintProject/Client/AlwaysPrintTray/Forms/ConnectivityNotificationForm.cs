...
namespace AlwaysPrintTray.Forms
{
    ...
    /// ZERO valores hardcodeados: todo se controla desde el alwaysconfig (texto, color, timeout, visibilidad).
    /// </summary>
    public sealed class ConnectivityNotificationForm : Form
    {
        // === COLORES DE ICONOS (visuales, no configurables) ===
        ...
        /// Toda la personalización proviene de notifConfig (desde el alwaysconfig).
        /// </summary>
        private ConnectivityNotificationForm(
            List<UrlCheckResult> results, int percent,
            ConnectivitySeverity severity, NotificationLevel notifConfig)
        {
            ...
            BackColor = ParseHexColor(notifConfig.Color);
            Opacity = 0; // Inicia invisible para el fade-in

            // Posicionar en esquina inferior derecha (sobre barra de tareas)
            PosicionarEnEsquina();

            // === Construir controles de UI con texto del config ===
            ConstruirUI(notifConfig);

            // === Timer de fade-in (300ms total) ===
            _fadeInTimer = new Timer { Interval = FadeStepMs };
            _fadeInTimer.Tick += OnFadeInTick;

            // === Timer de auto-cierre desde config (0 = persistente, sin auto-cierre) ===
            _autoCloseTimer = new Timer();
            if (notifConfig.TimeoutSeconds > 0)
            {
                _autoCloseTimer.Interval = notifConfig.TimeoutSeconds * 1000;
        ...
        /// <param name="payload">Payload original (para referencia futura).</param>
        /// <param name="severity">Nivel de severidad calculado por el handler.</param>
        /// <param name="notifConfig">Configuración de notificación para este nivel (desde alwaysconfig).</param>
            ...
            ConnectivitySeverity severity, NotificationLevel notifConfig)
        {
            ...
            var form = new ConnectivityNotificationForm(results, percent, severity, notifConfig);
            Current = form;
            form.Show();
            form.BringToFront();
            form.Activate();
            ...
            // Auto-cierre: solo si timeout > 0 (configurado en constructor)
            if (notifConfig.TimeoutSeconds > 0)
            {
                form._autoCloseTimer.Start();
            }
            // timeout_seconds == 0: persistente (sin auto-cierre)
        }
        ...
        /// Texto proviene directamente del alwaysconfig (notifConfig.Text).
        /// </summary>
        private void ConstruirUI(NotificationLevel notifConfig)
        {
            ...
            // === Label con texto del resultado (desde config) ===
            ...
            // === Botón "OK" / "Entendido" (persistente = timeout 0) ===
            bool isPersistent = notifConfig.TimeoutSeconds == 0;
        ...
        /// Los iconos son visuales y no requieren configuración desde alwaysconfig.
        /// </summary>
        ...
        /// <summary>
        /// Parsea un color hexadecimal (#RRGGBB). Fallback a gris claro si el formato es inválido.
        /// </summary>
        private static Color ParseHexColor(string hex)
        {
            try
            {
                if (string.IsNullOrEmpty(hex)) return Color.FromArgb(0xF5, 0xF5, 0xF5);
                return ColorTranslator.FromHtml(hex);
            }
            catch
            {
                return Color.FromArgb(0xF5, 0xF5, 0xF5);
            }
        }
